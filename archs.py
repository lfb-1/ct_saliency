...
class CTViT_Encoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        # self.var_head = nn.Sequential(nn.LayerNorm(dim), nn.Linear(dim, 1))
    ...
    def forward(
        ...
        *_, h, w, _ = shape
        ...
        # log_var = self.var_head(tokens)
        # log_var = (
        #     F.softplus(self.var_head(tokens) + np.log(np.exp(1.0 - 1e-8) - 1)) + 1e-8
        # ).clamp(1e-8, 1)
        return logits
...
if __name__ == "__main__":
    ...
    new_pt = extract_encoder(
        "../saved_weights/model_best_0.7623780916901571.pth.tar")
